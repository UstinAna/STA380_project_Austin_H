...
server <- function(input, output, session) {
  ...
  cancel_id <- reactiveVal(0)  # set to run_id() value when that run is cancelled
  tabs_rendered <- reactiveVal(character(0))

  # A run is cancelled if cancel_id matches run_id
  ...
  observeEvent(input$run, {
    ...
    run_id(run_id() + 1)
    # cancel_id is NOT reset — incrementing run_id means cancel_id no longer
    # matches, so is_cancelled() is automatically FALSE for the new run.
  })

  observeEvent(input$cancel, {
    cancel_id(run_id())  # stamp current run as cancelled
    hide_overlay()
  }, ignoreInit = TRUE)
  ...
  # ── Core Reactives ─────────────────────────────────────────────────────
  # No bindEvent anywhere — all reactives depend on run_id() directly.
  # This means when run_id increments (always a new value), they always
  # invalidate and re-execute, even after a cancel+rerun sequence where
  # input$run may have been batched/swallowed by Shiny's queue.
  ...
  output$mc_interpretation <- renderUI({
    ...
    if (delta == 0) {
      calib_perm <- ifelse(abs(perm_r - alpha) < 0.03,
                           "well-calibrated", "slightly miscalibrated")
      calib_t    <- ifelse(abs(t_r - alpha) < 0.03,
                           "well-calibrated", "slightly miscalibrated")
      tagList(
        tags$b("Interpretation (\u03b4 = 0 \u2192 Type I error):"),
        tags$ul(
          tags$li(paste0("Permutation test rejects ",
                         round(perm_r * 100, 1), "% \u2014 ", calib_perm,
                         " (nominal \u03b1 = 5%)")),
          tags$li(paste0("t-test rejects ",
                         round(t_r * 100, 1), "% \u2014 ", calib_t,
                         " (nominal \u03b1 = 5%)"))
        )
      )
    } else {
  ...
  output$power_curve_plot <- renderPlot({
    ...
    df <- power_curve_res()
    df_long <- rbind(
      data.frame(delta = df$delta, power = df$t_test,
                 Test = "t-test"),
      data.frame(delta = df$delta, power = df$permutation_test,
    ...
    p <- ggplot(df_long, aes(x = delta, y = power, colour = Test, group = Test)) +
      ...
      geom_hline(yintercept = 0.05, linetype = "dashed",
                 colour = "gray50", linewidth = 0.8) +
      annotate("text", x = 0, y = 0.07, label = "\u03b1 = 0.05",
               colour = "gray50", hjust = 0, size = 3.5) +
      scale_colour_manual(values = c("t-test" = "#F8A04B",
                                     "Permutation test" = "#6C63FF")) +
      scale_y_continuous(limits = c(0, 1), labels = scales::percent) +
      labs(x = "Effect size (\u03b4)", y = "Rejection rate",
           title = paste0("Power Curve — ", input$dist,
                          " distribution, n = ", input$n),
           colour = "Test") +
      theme_minimal(base_size = 13) +
      theme(plot.title = element_text(face = "bold", colour = "#6C63FF"),
            legend.position = "top")
  ...
  output$robust_plot <- renderPlot({
    ...
    df <- rbind(
      data.frame(Distribution = type1$Distribution,
                 t_test = type1$t_test,
                 permutation_test = type1$permutation_test,
                 Scenario = "Type I Error (\u03b4=0)"),
      data.frame(Distribution = power$Distribution,
                 t_test = power$t_test,
    ...
    p <- ggplot(df_long, aes(x = Distribution, y = Rate,
                             fill = Test, group = Test)) +
      geom_col(position = "dodge", alpha = 0.85, width = 0.6) +
      geom_hline(yintercept = 0.05, linetype = "dashed",
                 colour = "gray40", linewidth = 0.8) +
      scale_fill_manual(values = c("t-test" = "#F8A04B",
                                   "Permutation test" = "#6C63FF")) +
      scale_y_continuous(limits = c(0, 1), labels = scales::percent) +
      facet_wrap(~Scenario, scales = "free_y") +
      labs(x = "Distribution", y = "Rejection rate",
           title = "Robustness: Rejection Rates Across Distributions",
           fill  = "Test") +
      theme_minimal(base_size = 13) +
      theme(plot.title = element_text(face = "bold", colour = "#6C63FF"),
            legend.position = "top")
  ...
  output$download <- downloadHandler(
    filename = function() {
      paste0("data_", input$dist, "_delta", input$delta, ".csv")
    },
